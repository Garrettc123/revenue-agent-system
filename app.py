...
Clean Flask entrypoint with Stripe revenue telemetry, checkout/webhook hooks,
master-conductor integration, SSE events, and the Funnel Control Plane.
"""
import os
import queue
import threading
...
try:
    from cache_utils import cached, invalidate_revenue_cache, TTL_STRIPE_REVENUE, TTL_CONDUCTOR
except ImportError:  # pragma: no cover
...
try:
    from funnel_control.routes import funnel_blueprint
except ImportError:  # pragma: no cover
    funnel_blueprint = None
...
def _notify_sse(event: str, data: dict[str, Any]) -> None:
    ...
    with _sse_lock:
        for subscriber in list(_sse_subscribers):
            try:
...
<html><head><title>Revenue Agent Dashboard</title></head>
<body>
<h1>Revenue Agent Dashboard</h1>
<section><h2>Monthly Recurring Revenue</h2><div id="mrr">$0</div></section>
<section><h2>Active Customers</h2><div id="customers">0</div></section>
<section><h2>System Status</h2><div id="status">ONLINE</div></section>
<script>
async function updateDashboard() {
  const r = await fetch('/api/revenue');
  const d = await r.json();
  document.getElementById('mrr').textContent = '$' + Number(d.mrr).toLocaleString();
    ...
    try:
        ...
        for subscription in stripe.Subscription.list(status="active", limit=100).auto_paging_iter():
            for item in subscription["items"]["data"]:
                ...
                if price.get("recurring", {}).get("interval") == "year":
                    amount /= 12
...
def sync_revenue():
    ...
    return jsonify({"status": "success", "data": fetch_stripe_revenue(), "timestamp": datetime.utcnow().isoformat()})
...
def conductor_dashboard():
    return jsonify(conductor.get_master_dashboard() if conductor else {"status": "unavailable"})


@app.route("/api/conductor/financial-summary")
def conductor_financial_summary():
    return jsonify(conductor.get_financial_summary() if conductor else {"status": "unavailable"})


@app.route("/api/conductor/forecast")
def conductor_forecast():
    months = request.args.get("months", 12, type=int)
    return jsonify(conductor.get_revenue_forecast(months) if conductor else {"status": "unavailable"})


@app.route("/api/conductor/health")
def conductor_health():
    return jsonify(conductor.get_system_health() if conductor else {"status": "unavailable"})
...
def events_stream():
    def generate():
        ...
        try:
            ...
            while True:
                try:
                    yield subscriber.get(timeout=25)
                except queue.Empty:
                    yield ": keep-alive\n\n"
...
if funnel_blueprint is not None:
    app.register_blueprint(funnel_blueprint)
